<link rel="stylesheet" href="/gruppo10/global.css">
<link rel="stylesheet" href="/gruppo10/home/banner_home/banner_home.css">

    <div class="bg">
        <video id="bg-video" src="/gruppo10/resources/video_prova.mp4" autoplay muted loop disablePictureInPicture></video>

        <div class="bn-intro">
            <div class="content">
                <p id="intro">Cerca ora la tua <span id="key">ispirazione</span>...</p>
                <p id="sub">Cerca qui la tua ricetta e scopri una lista di piatti scelti e condivisi dagli utenti migliori.<br>Crea il tuo piatto, crea la tua storia </p>
            </div>

            <?php
            include('../home/banner_home/search-bar.php')
            ?>

        </div>

    </div>
